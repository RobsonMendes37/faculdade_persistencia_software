...
import { createProduto } from '../../services/ProdutoServicer';
...
const AddProduto = () => {
    ...
    const saveProduto = (e) => {
        ...
        const produto = {
            ...
            peso, 
            tamanho,
            preco: parseFloat(preco), // Certifique-se de que o preço seja um número
            quantidade: parseInt(quantidade), // Certifique-se de que a quantidade seja um número inteiro
            fornecedor,
            marca
        };
        ...
        .catch((error) => {
            ...
            if (error.response && error.response.data && error.response.data.detail) {
                setErrorMessage(`Erro ao salvar produto: ${error.response.data.detail}`);
            } else {
                setErrorMessage('Erro ao salvar produto: Erro desconhecido');
            }
        });
    ...
    const validateForm = () => {
        ...
        if (!preco.trim() || isNaN(preco) || parseFloat(preco) <= 0) {
    ...
    // Título da página
    function pageTitle() {
        return <h2 className='text-center'>Adicionar Produto</h2>;
    }
